...
def unload_provider_modules() -> None:
    """Remove all provider modules from sys.modules to test fresh imports."""
    modules_to_remove = [
        key
        for key in sys.modules
        if key.startswith("mirascope.llm.providers.")
        and not key.startswith("mirascope.llm.providers.base")
    ]
    for module in modules_to_remove:
        del sys.modules[module]


def stub_package(package_name: str) -> None:
    """Add a package to sys.modules as None to simulate missing dependency."""
    # Store original if it exists
    if package_name in sys.modules:
        # Don't actually remove it in tests since we have the deps installed
        pass
    # For testing, we'll use the _stubs module directly
...
class TestMissingDependencies:
    ...
    def test_stub_class_can_be_subclassed_in_definition(self) -> None:
        ...
        class MyClass(stub_class):  # type: ignore
            pass
    ...
    def test_stub_class_can_be_used_in_type_hints(self) -> None:
        """Test that stub classes can be used in type hints without failing."""
        from mirascope._stubs import _StubModule
        ...
        def function_with_hint(x: stub_class) -> stub_class:  # type: ignore
            return x

        # Should be able to pass around the class itself
        classes = [stub_class, stub_class]
    ...
    def test_nested_import_via_finder(self) -> None:
        ...
        try:
            ...
            import test_finder_stub.nested.deep  # type: ignore
            ...
            something = test_finder_stub.nested.deep.SomeClass  # type: ignore
            ...
            sys.modules["test_non_stub_parent"] = object()  # type: ignore
...
class TestProviderStubs:
    ...
    def test_providers_can_be_used_in_subclass_definitions(self) -> None:
        ...
        class MyCustomProvider(AnthropicProvider):  # type: ignore
