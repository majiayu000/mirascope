...
# IMPORTANT: Keep this in sync with [project.optional-dependencies] in pyproject.toml
# When adding/modifying extras, update this mapping accordingly
# Note: Multiple PyPI packages may provide the same namespace (e.g., opentelemetry-api
# and opentelemetry-sdk both provide 'opentelemetry'), so we only check once.
EXTRA_IMPORTS: dict[str, list[str]] = {
    "anthropic": ["anthropic"],
    "api": ["pydantic_settings"],
    "google": ["google.genai", "PIL", "proto"],  # google-genai, pillow, proto-plus
    "openai": ["openai"],
    "mcp": ["mcp"],
    "ops": [
        "opentelemetry",  # covers opentelemetry-sdk and opentelemetry-api
